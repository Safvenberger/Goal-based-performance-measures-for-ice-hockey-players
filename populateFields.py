...
from matchDate import parse_data, fix_match_date, add_date_to_game, add_date_to_pbp


def create_game(connection, season, game_table="mgame"):
    """
    Create a new table (default 'mgame') to contain metadata about each game.
    ...
    game_table : string.
        Name of the metadata game SQL table to be altered. The default is "mpbp".
    ...
    drop_query = f"DROP TABLE IF EXISTS {game_table}"
    cursor.execute(drop_query)

    # Create a copy of the pre-existing 'game' table 
    query = f"""CREATE TABLE {game_table} AS SELECT * FROM game 
                WHERE GameId LIKE '{season}%'"""
    cursor.execute(query)

    # Add new columns
    query = f"""ALTER TABLE {game_table} 
            ADD AwayTeamId INT, ADD HomeTeamId INT, 
            ADD GoalsAgainst INT, ADD GoalsFor INT, ADD Outcome TEXT"""
...
def cut_play_by_play(connection, season, start_date=None, end_date=None, 
    ...
    # Multiple seasons
        ...
        query += f"""WHERE GameId > {season}020000 AND GameId < 2014020000 AND 
                           GameId LIKE '20__02%'"""
    # One full season without dates. Doesnt work with partitioning
    else: 
        query += f"WHERE GameId LIKE '{season}%'"
    cursor.execute(query)
...
def add_gf_and_ga_fast(connection, pbp_table="mpbp"):
    ...
    query = f"""CREATE TABLE mpbp_temp
            ...
            FROM {pbp_table} g,
            (SELECT @home_team_score := 0) x,
            (SELECT @away_team_score := 0) y,
            (SELECT @curr_goal := '') g,
            (SELECT @prev_game := 0) v,
            (SELECT @curr_game := 0) w,
            (SELECT @prev_home := '') ph,
            (SELECT @prev_away = '') pa,
            (SELECT @curr_home = '') ch,
            (SELECT @curr_away = '') ca) as rowTable) as results;"""
...
def extract_season(connection, engine, season, 
                   ...
                   get_dates=False, add_dates=False, 
                   multiple_seasons=False, pbp_table="mpbp"):
    """
    ...
    """
    print(f"Extracting season: {season}-{season+1} and popuplating fields.")
    #print("Creating game table...")
    create_game(connection, season)
    if get_dates:
        #print("Getting dates...")
        parse_data()
        fix_match_date(connection)

    if add_dates:
        #print("Adding dates...")
        add_date_to_game(connection)
        add_date_to_pbp(connection)
...
if __name__ == "__main__":
    current_season = 2012
    db_connection = connect_to_db("hockey")
    engine = create_db_engine("hockey")
    extract_season(db_connection, engine, current_season, 
